...
raylibr provides functions for keyboard, mouse, gamepad, and touch
input. Input state is updated each frame, so these functions are meant
to be called inside a game loop.
...
to consume queued key events or
...
By default, pressing Escape closes the window. Change or disable this
with
...
Common key codes: `key$space`, `key$enter`, `key$escape`, `key$up`,
`key$down`, `key$left`, `key$right`, `key$a` through `key$z`.
...
Available cursors: `mouse_cursor$default`, `mouse_cursor$arrow`,
`mouse_cursor$ibeam`, `mouse_cursor$crosshair`,
`mouse_cursor$pointing_hand`, `mouse_cursor$resize_ew`,
`mouse_cursor$resize_ns`, `mouse_cursor$resize_all`,
...
For complete interactive input examples, see the [Mouse
Test](https://jeroenjanssens.github.io/raylibr/articles/demo-mouse.md)
and
[Snake](https://jeroenjanssens.github.io/raylibr/articles/demo-snake.md)
demos.
