...
`camera_2d()` creates a 2D camera with four properties:
...
Wrap your drawing calls in `begin_mode_2d()` / `end_mode_2d()` to apply the camera transform. Anything drawn outside this block uses screen coordinates directly — useful for HUD elements.

```{r camera-2d, eval=CAN_RENDER}
...
The rectangle and circles are drawn in world coordinates. The camera zooms in 2x and centers on `(100, 100)`, so they appear magnified. The text is drawn outside the camera block, so it stays fixed on screen.
...
All parameters except `position` have sensible defaults. Wrap 3D drawing in `begin_mode_3d()` / `end_mode_3d()`:

```{r camera-3d-close, eval=CAN_RENDER}
raylibr_screenshot(function() {
...
The helper `raylibr_screenshot_3d()` provides a shortcut — it sets up a camera at `c(4, 4, 4)`, draws a grid, and wraps your drawing function automatically.
...
```{r orthographic, eval=CAN_RENDER}
raylibr_screenshot(function() {
...
For interactive applications, `update_camera()` provides four built-in control schemes:
...
`update_camera()` modifies the camera in place — no need to reassign.
