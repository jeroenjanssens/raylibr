...
A complete Snake game built entirely in R: arrow keys steer the snake, eating food grows it, hitting a wall or its own body resets the game. It demonstrates keyboard input via `get_key_pressed()` and the `key` enum, grid-based collision detection using R lists, `draw_rectangle_v()` for rendering the board, and `set_target_fps()` to control game speed.
...
<iframe src="../wasm/snake.html" width="600" height="480" style="border:none; display:block; margin:0 auto;"></iframe>
