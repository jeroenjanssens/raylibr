...
An interactive Julia set fractal viewer powered by a GLSL fragment shader: press keys 1–6 to jump between six classic points of interest, click and drag to pan, and scroll to zoom. The fractal is rendered into a `render_texture` each frame, then displayed via `draw_texture_ex()` with the shader active. It exercises `load_render_texture()`, `begin_texture_mode()`, `target$texture` field access, `set_shader_value()` for per-frame uniforms, and mouse input.

## Try it

Click the canvas to focus it. Left-click to zoom in, right-click to zoom out and pan. Press keys 1–6 to jump to different points of interest, Space to pause, Left/Right to change animation speed.

<iframe src="../wasm/julia.html" width="600" height="460" style="border:none; display:block; margin:0 auto;"></iframe>
