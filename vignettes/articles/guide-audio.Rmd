...
raylibr supports sound effects and music playback through Raylib's audio system. The audio device is automatically initialized when you call `init_window()`.

## Sound effects

Sounds are loaded entirely into memory -- use them for short clips like explosions, clicks, or notifications.
...
Adjust volume (0.0 to 1.0), pitch (1.0 = normal), and stereo pan (0.0 = left, 0.5 = center, 1.0 = right):
...
Music streams read audio data in small chunks, making them suitable for longer tracks like background music. The key difference from sounds: you must call `update_music_stream()` every frame to feed the audio buffer.
...
If you need audio without a window, initialize the audio device directly:
...
For complete audio examples, see the [Beatbox](demo-beatbox.html) demo (sound effects) and the [Waving Cubes](demo-cubes.html) demo (music stream).
